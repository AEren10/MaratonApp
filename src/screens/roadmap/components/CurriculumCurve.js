...
function CurriculumCurve({ ratio = 0 }) {
  ...
  return (
    <Svg viewBox="0 -8 300 76" width="100%" style={{ aspectRatio: 300 / 76 }}>
      <Path d={FULL} fill="none" stroke={C.track} strokeWidth={2.4} strokeLinecap="round" />
      {r > 0 ? <Path d={done} fill="none" stroke={C.accent} strokeWidth={4} strokeLinecap="round" /> : null}
      <Circle cx={6} cy={58} r={4} fill={C.bg} stroke={C.accent} strokeWidth={2.2} />
      {nodes.map(({ f, p }) => (
        <Circle key={f} cx={p[0]} cy={p[1]} r={4} fill={C.bg} stroke={f <= r ? C.accent : C.stop} strokeWidth={2.2} />
      ))}
      <Circle cx={294} cy={8} r={5} fill={C.bg} stroke={C.projNode} strokeWidth={2.2} />
      <Circle cx={here[0]} cy={here[1]} r={6.5} fill={C.accent} />
      <SvgText x={labelX} y={here[1] > 40 ? here[1] - 12 : here[1] + 20}textAnchor="middle" fontFamily="Archivo_700" fontSize={11} letterSpacing={1.4} fill={C.accent}>
        BURADASIN
      </SvgText>
    </Svg>
  );
