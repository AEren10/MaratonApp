    ...
    >
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <Pressable onPress={onEdit} hitSlop={12} style={editBtn}>
          <Icon name={noGoal ? "target" : "edit"} size={13} color={C.textOnFill} />
          <Text style={[editText, { color: C.textOnFill }]}>{noGoal ? "Belirle" : "Güncelle"}</Text>
        </Pressable>
      </View>
      ...
      <View style={{ flexDirection: "row", alignItems: "center", gap: SPACING.lg, marginTop: SPACING.lg }}>
        ...
        >
          <Text style={[ringPct, { color: C.textOnFill }]}>%{Math.round(pct * 100)}</Text>
        </ProgressRing>

        <View style={{ flex: 1, gap: 4 }}>
          <Text style={[rankText, { color: C.textOnFill }]}>{label}</Text>
      ...
      <View style={{ flexDirection: "row", marginTop: SPACING.lg, gap: SPACING.md }}>
        <View style={statBox}>
          <Text style={[statNum, { color: C.textOnFill }]}>{elapsed}</Text>
          <Text style={statLabel}>gün geçti</Text>
        </View>
        <View style={statBox}>
          <Text style={[statNum, { color: C.textOnFill }]}>{daysUntilExam || 0}</Text>
