-- Scope route week snapshots by exam type.
--
-- route_weeks already stores exam_type and read paths filter by it, but the
-- old unique index/upsert target was only (user_id, week_start). Switching
-- from YKS to LGS in the same calendar week could therefore overwrite the old
-- exam's weekly plan snapshot. Keep NULL legacy rows distinct-safe too.

CREATE UNIQUE INDEX IF NOT EXISTS idx_route_weeks_user_exam_week
  ON public.route_weeks (user_id, exam_type, week_start) NULLS NOT DISTINCT;

DROP INDEX IF EXISTS public.idx_route_weeks_user_week;

CREATE INDEX IF NOT EXISTS idx_route_weeks_user_exam_time
  ON public.route_weeks (user_id, exam_type, week_start DESC);
